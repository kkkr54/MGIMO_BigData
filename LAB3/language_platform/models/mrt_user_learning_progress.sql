...
with source_data as (
    ...
    join {{ ref('lessons') }} les on p.lesson_id = les.lesson_id
    join {{ ref('languages') }} l on les.lang_id = l.lang_id
    left join {{ ref('dictionaries') }} d on u.user_id = d.user_id and l.lang_id = d.lang_id
    left join {{ ref('exercises') }} ex on les.lesson_id = ex.lesson_id
)
